...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const commandItems = useMemo(() => [
    ...
    { name: "Public Booking Page", href: "/book/maya", icon: "external" as const },
  ...
  const currentPage = links.find(([, href]) => href === pathname)?.[0] ?? (pathname === "/dashboard/settings" ? "Settings" : "Dashboard");
    ...
    <div className="app-shell">
      <aside className={`app-sidebar ${menuOpen ? "open" : ""}`}>
        <div className="sidebar-brand">
          ...
          <button className="icon-button mobile-close" onClick={() => setMenuOpen(false)} aria-label="Close menu"><Icon name="close" /></button>
        </div>
        <div className="workspace-label">Workspace</div>
        <nav className="app-nav" aria-label="Dashboard">
          {links.map(([name, href, icon]) => (
            <Link className={pathname === href ? "active" : ""} href={href} onClick={() => setMenuOpen(false)} key={href}>
              <Icon name={icon} />
              {name}
            </Link>
          ))}
        ...
        <div className="sidebar-bottom">
          ...
          <Link className="public-page-link" href="/book/maya">
            <Icon name="external" />
      ...
      <div className="app-main">
        <header className="app-topbar">
          <div className="topbar-left">
            <button className="icon-button menu-button" onClick={() => setMenuOpen(true)} aria-label="Open menu"><Icon name="menu" /></button>
            {pathname !== "/dashboard" && <Link className="back-link dashboard-back" href="/dashboard">← Back to Overview</Link>}
            <span className="topbar-current">{currentPage}</span>
          </div>
          <button className="topbar-search" onClick={openCommand}>
            <Icon name="search" size={16} />
            <span>Search and navigate</span>
            <kbd>⌘ K</kbd>
          </button>
          <div className="topbar-actions">
            <button className="icon-button" onClick={() => setNotificationsOpen((open) => !open)} aria-label="Notifications" aria-expanded={notificationsOpen}><Icon name="bell" size={17} /><span className="notification-dot" /></button>
            <Link className="button compact-button" href="/book/maya"><Icon name="external" size={15} />Public Page</Link>
            {notificationsOpen && <div className="notification-popover" role="status"><strong>You’re all caught up</strong><p>New bookings and changes will appear here.</p></div>}
          </div>
        </header>
          ...
          <section className="command-menu" role="dialog" aria-modal="true" aria-label="Command menu" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="command-results">
              {commandResults.length ? commandResults.map((item, index) => (
                <button className={index === commandIndex ? "selected" : ""} onMouseEnter={() => setCommandIndex(index)} onClick={() => selectCommand(item.href)} key={item.href}>
                  <Icon name={item.icon} size={16} />
                  <span>{item.name}</span>
                  {pathname === item.href && <small>Current</small>}
                </button>
              )) : <div className="command-empty"><strong>No results</strong><span>Try another page or action.</span></div>}
            </div>
