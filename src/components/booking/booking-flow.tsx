...
export function BookingFlow({ slug }: { slug: string }) {
  ...
  return <div className="booking-frame">
    <aside className="booking-summary">
      ...
      <div className="booking-note"><span>✦</span><p>Times are displayed in your local timezone. You’ll receive an email confirmation immediately.</p></div>
    </aside>
    ...
    <section className="booking-panel">
      ...
      {step === "datetime" && <div className="booking-stage">
        <button className="text-button" onClick={() => setStep("service")}>← Change service</button>
      ...
      {step === "details" && <div className="booking-stage">
        <button className="text-button" onClick={() => setStep("datetime")}>← Choose another time</button>
        ...
        <form className="booking-form" action={submit}>
          <div className="field-grid"><label>Full name<input name="name" autoComplete="name" placeholder="Ada Lovelace" required /></label><label>Email address<input name="email" type="email" autoComplete="email" placeholder="ada@example.com" required /></label></div>
          <label>Anything Maya should know? <span className="optional">Optional</span><textarea name="note" rows={4} placeholder="A little context helps make the session more useful…" /></label>
          <label className="checkbox-row"><input type="checkbox" required /><span>I agree to receive emails about this appointment.</span></label>
